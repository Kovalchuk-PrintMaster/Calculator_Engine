...
def upgrade():
    ...
    op.execute("ALTER TABLE product_kind_print_colors DROP CONSTRAINT product_kind_print_colors_pkey;")
    # додати id та required
    op.add_column("product_kind_print_colors", sa.Column("id", sa.BigInteger, primary_key=True, autoincrement=True))
    op.add_column("product_kind_print_colors", sa.Column("required", sa.Boolean, nullable=False, server_default=sa.text("FALSE")))
