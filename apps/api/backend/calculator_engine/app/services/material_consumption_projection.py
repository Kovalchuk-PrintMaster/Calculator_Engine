...
def build_draft_material_consumption_estimate(*, draft_id: str):
	...
	from catalog.services import build_price_quote, build_material_consumption_estimate
	from catalog.services import assert_projection_usage
	...
	if not draft.product_template_code or not draft.material_code or not draft.quantity:
		raise MaterialConsumptionProjectionValidationError(
			"Draft is not ready for material consumption estimate."
		)

	template = ProductTemplate.objects.filter(code=draft.product_template_code, active=True).first()
	if template is None:
		raise MaterialConsumptionProjectionValidationError(
			f"ProductTemplate not found: {draft.product_template_code}"
		)

	material = Material.objects.filter(code=draft.material_code, active=True).first()
	if material is None:
		raise MaterialConsumptionProjectionValidationError(
	...
	requested_quantity = int(material_line.meta.get("requested_quantity", draft.quantity))
...
def build_calculation_job_material_consumption_estimate(*, job_public_id: str):
	...
	if material_line is None:
		raise MaterialConsumptionProjectionValidationError("Material line not found in calculation job.")

	requested_quantity = int(material_line.get("meta", {}).get("requested_quantity", job.quantity))
